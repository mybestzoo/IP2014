...
\begin{align*}
|G_{k,d}|=\frac{|S^{d-1}||S^{d-2}|\dots|S^{d-k}|}{2|S^{k-1}||S^{k-2}\dots|S^1||},& k\ge2,
\\|G_{1,d}|=|S^{d-1}|/2,
\\|G_{0,n}|=1.
\end{align*}
By $\pi^\perp$ denote the $(d-k)$-dimensional orthogonal complement to $\pi\in G_{k,d}$. The integral over $\mathbb R^d$ can be presented as
\begin{equation}
\label{integral}
